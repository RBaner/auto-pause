...
async def toggle_spotify():
    ...
    while True:
        ...
        if current_state_chrome == previous_state_chrome:#status of 4 is playing, 5 is paused
            time.sleep(1)
            continue
        elif current_state_chrome == 5 and previous_state_chrome == 4: #just paused
            if previous_state_spotify == 4:
                await spotify_manager.try_toggle_play_pause_async()
            previous_state_chrome = current_state_chrome
            time.sleep(1)
            continue
        elif current_state_chrome == 4 and previous_state_chrome == 5: #just started playing
